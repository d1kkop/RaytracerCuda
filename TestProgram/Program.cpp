...
namespace TestProgram
{
    ...
    void Program::initialize(const string& name, u32 width, u32 height)
    {
        ...
        m_tempBuff = new u32[width*height];
   ...
   //     CUDA_CALL( cudaSetDevice( count-1 ) );
        ...
        assert(succes);
        succes = m_textureBufferObject.init(width, height);
        assert(succes);
        ...
        //vertices[0] = vec4(-1.f, -1.f, 1.f, 1);
        //vertices[1] = vec4(0.f, 1.f, 1.f, 1);
        //vertices[2] = vec4(1.f, -1.f, 1.f, 1);
        //vertices[3] = vec4(1.f, 1.f, 1.f, 1);
        ...
        //assert(err==0);
        //err = mesh->setVertexData((float*)vertices, 4, 4, VERTEX_DATA_POSITION);
        //assert(err==0);
...
//        u32 err;
   ...
   //     m_scene->updateGPUScene();
        ...
        // -- Clear --
        err = m_camera->clear( m_textureBufferObject.renderTarget(), 255<<16 );
        assert(err==0);

   //     m_blob->run( Util::time() );
   //     m_blob->transfer( m_tempBuff );
   //     glDrawPixels( 1200, 800, GL_RGBA, GL_UNSIGNED_BYTE, m_tempBuff );

        // -- Trace scene --
        //vec3 eye    = vec3(0, 0, -2.f);
        //mat3 orient = mat3(1.f);
        //err = m_camera->traceScene(&eye.x, &orient[0][0], m_scene, m_textureBuffer.renderTarget());
        //assert(err==0);

        // -- Render texture to backbuffer
        m_glRenderer.render( m_textureBufferObject );
